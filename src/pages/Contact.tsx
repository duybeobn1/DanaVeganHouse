  ...
  return (
    <main>
      ...
      <section className="relative pt-20 pb-16 md:pt-28 md:pb-20 rice-texture overflow-hidden">
        <div className="absolute inset-0" style={{ backgroundImage: 'url(/images/farmers_green_wallpaper.png)', backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }} />
      ...
      <section className="bg-cream py-20 md:py-40">
        <div className="max-w-5xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <div className="space-y-10">
            ...
            <Reveal delay={0.08}>
              <div>
                <p className="text-xs font-bold tracking-[0.3em] uppercase text-brand mb-4">{t('Giờ Mở Cửa', 'Opening Hours')}</p>
                <div className="space-y-2 text-earth-mid text-sm">
                  <div className="flex justify-between border-b border-rice-dark pb-2">
                    <span className="font-semibold text-earth">{t('Thứ 2 – Thứ 6', 'Mon – Fri')}</span>
                    <span>11:00 – 22:00</span>
                  </div>
                  <div className="flex justify-between border-b border-rice-dark pb-2">
                    <span className="font-semibold text-earth">{t('Thứ 7', 'Saturday')}</span>
                    <span>11:00 – 22:00</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="font-semibold text-earth">{t('Chủ Nhật', 'Sunday')}</span>
                    <span>11:00 – 22:00</span>
                  </div>
                </div>
              </div>
            ...
            <Reveal delay={0.15}>
              <div>
                <p className="text-xs font-bold tracking-[0.3em] uppercase text-brand mb-4">{t('Đặt Bàn & Liên Hệ', 'Reservations & Enquiries')}</p>
                <a href="tel:0828744931" className="text-3xl font-black text-earth hover:text-brand transition-colors">0828.744.931</a>
                <p className="text-earth/50 text-sm mt-2">
                  {lang === 'vi' ? (
                    <>Gọi, nhắn tin hoặc liên hệ trang{' '}
                      <a href="https://www.facebook.com/danaveganhouse" target="_blank" rel="noreferrer" className="underline hover:text-earth transition-colors">Facebook</a>
                      {' / '}
                      <a href="https://www.instagram.com/danaveganhouse/" target="_blank" rel="noreferrer" className="underline hover:text-earth transition-colors">Instagram</a>{' '}
                      để đặt bàn</>
                  ) : (
            ...
            <Reveal delay={0.2}>
              <div>
                <p className="text-xs font-bold tracking-[0.3em] uppercase text-brand mb-4">{t('Tìm Chúng Tôi', 'Find Us On')}</p>
                <div className="flex flex-wrap gap-3">
                  {[
                    ...
                    { label: 'Google Maps', href: 'https://www.google.com/maps/place/Dana+Vegan+House' },
                  ].map(({ label, href }) => (
                    <a key={label} href={href} target="_blank" rel="noreferrer"
                      className="text-xs font-bold tracking-wider uppercase border border-earth/30 text-earth-mid px-4 py-2 rounded-full hover:border-brand hover:text-brand transition-all duration-300 btn-magnetic">
                      {label}
                    </a>
                  ))}
                </div>
          ...
          <div className="space-y-6">
            ...
            <Reveal delay={0.15}>
              <div className="bg-rice rounded-[2rem] p-6 shadow-ambient">
                <p className="text-xs font-bold tracking-[0.3em] uppercase text-brand mb-3">{t('Lưu Ý', 'Note')}</p>
                <p className="text-earth/70 text-sm leading-relaxed">{t('Dāna Vegan House nằm trên phố Đông Thái nhỏ, gần Chợ Gạo, trong khu phố cổ Hàng Buồm, Hoàn Kiếm. Tìm biển hiệu "dāna VEGAN HOUSE".', 'Dāna Vegan House is located on the small Đông Thái Street, near the Rice Market, in the historic Hàng Buồm neighborhood, Hoàn Kiếm. Look for the "dāna VEGAN HOUSE" sign.')}</p>
              </div>
            </Reveal>

            <Reveal delay={0.2}>
              <div className="bg-earth rounded-[2rem] p-6 shadow-premium-dark">
                <p className="text-gold font-black text-base mb-3">{t('Kết Nối Cùng Dāna', 'Connect with Dāna')}</p>
                <p className="text-cream/60 text-sm leading-relaxed mb-4">{t('Hãy cùng Dāna lan toả tinh thần chay đổi mới và văn hóa ẩm thực Việt Nam thông qua những trải nghiệm thú vị của bạn!', 'Join Dāna in spreading the spirit of modern veganism and Vietnamese food culture through your experiences!')}</p>
                <div className="flex gap-2">
                  <a href="https://www.instagram.com/danaveganhouse/" target="_blank" rel="noreferrer"
                    className="text-xs font-bold tracking-wider uppercase text-gold border border-gold/30 px-4 py-2 rounded-full hover:bg-gold hover:text-earth transition-all duration-300 btn-magnetic">Instagram</a>
                  <a href="https://www.facebook.com/danaveganhouse" target="_blank" rel="noreferrer"
                    className="text-xs font-bold tracking-wider uppercase text-gold border border-gold/30 px-4 py-2 rounded-full hover:bg-gold hover:text-earth transition-all duration-300 btn-magnetic">Facebook</a>
                </div>
              </div>
            </Reveal>
          </div>
